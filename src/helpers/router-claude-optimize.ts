type RegistrationCallback = ((args: any, params: { [k: string]: string | string[] }) => void) | ((args: any) => void);
...
export class MatchRouter {
  ...
  activateRoute(pathname: `/${string}`, args: any, errorCallback: (err: Error) => void): void {
    pathname = this.getActualRoutePath(pathname);
    const segments = this.parsePath(pathname);
    let currentNode = this.root;
    ...
    if (currentNode.callback) {
      try {
        currentNode.callback(args, params);
      } catch (error) {
